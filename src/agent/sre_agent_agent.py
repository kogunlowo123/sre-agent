"""SRE Agent - Main Agent Implementation."""
...
class SreAgentAgent:
    ...
    def __init__(self):
        ...
        self.slo_management_enabled = True
        self.error_budgets_enabled = True
        self.capacity_planning_enabled = True
        self.toil_reduction_enabled = True
        self.chaos_engineering_enabled = True
        self._tool_dispatch = {
            "define_slo": self.tools.define_slo,
            "check_error_budget": self.tools.check_error_budget,
            "plan_capacity": self.tools.plan_capacity,
            "identify_toil": self.tools.identify_toil,
            "run_chaos_experiment": self.tools.run_chaos_experiment,
            "score_reliability": self.tools.score_reliability,
        }
        logger.info(
            "agent_initialized",
            ...
            features=['slo_management', 'error_budgets', 'capacity_planning', 'toil_reduction', 'chaos_engineering'],
        )
    ...
    async def _maybe_execute_tools(self, message: str, context: str) -> list[dict]:
        ...
        tool_keywords = {
            "define_slo": ['define', 'slo'],
            "check_error_budget": ['check', 'error', 'budget'],
            "plan_capacity": ['plan', 'capacity'],
            "identify_toil": ['identify', 'toil'],
            "run_chaos_experiment": ['run', 'chaos', 'experiment'],
